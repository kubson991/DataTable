<template>
<section>
  <h1>jaja salu2</h1>
  
  <button @click="()=>gridGlobalConfig.groupBy ? gridGlobalConfig.groupBy = null   : gridGlobalConfig.groupBy = 'gender' ">Group by gender </button>
  <button @click="deleteMaleRule">delete rows of MALES</button>
  <GridTable
    :tableConfig="gridGlobalConfig"
    :columns="columns"
    :data="data"
    :loading="tableLoader"
    :ajaxTable="false"
    @onChangeOrderPicked="manageOrderPicked"
    @onClickEvent="elementClicked"
    @deleteRows="(SetDeleteRowRule)=>deleteRow=SetDeleteRowRule"
    class="tableCostumClass"
  >
    <template #[`Genero`]>
      <div class="containImage">
        <img
          src="data:image/png;base64,"
          alt="image"
        />
      </div>
    </template>
    <template #[`GroupTitle`]="{group}">
      <img
        v-if="group === 'Masculino'"
        src="https://upload.wikimedia.org/wikipedia/commons/thumb/2/2d/Male-símbolo.svg/1024px-Male-símbolo.svg.png"
        alt=""
        class="fitImgSex"
      />
      <img
        v-else-if="group === 'Femenino'"
        src="https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/Female_symbol_%28heavy_pink%29.svg/1200px-Female_symbol_%28heavy_pink%29.svg.png"
        alt=""
        class="fitImgSex"
      />
      <span v-else> No Tiene Sexo</span>
    </template>
    <template #[`Column.customcolor`]="{ rowInfo }">
      <div
        class="ColorForPerson"
        :style="{
          'background-color': rowInfo.gender === 'Masculino' ? 'green' : 'pink',
          'background-image': (rowInfo.gender !== 'Masculino' && rowInfo.gender !== 'Femenino') && 'linear-gradient(to right top, #d476d8, #ed5a7e, #c86830, #827900, #1f7c2f)'
        }"
      ></div>
      
    </template>
  </GridTable>
  </section>
</template>
<script>
import api from "../mockDataInfo";
import GridTable from '../components/GridTable/GridTable.vue'
export default {
  components:{GridTable},
  data() {
    return {
      deleteRow:null,
      gridGlobalConfig: {
        groupBy: "gender",
        activeFilters:true,
        key:'document'
      },
      tableLoader: false,
      customFilterGender: null,
      quantityPerPage: 10,
      totalPages: 5,
      page: 1,
      columns: [
        {
          name: "Nombre",
          value: "name",
          orderable: true,
          type: "string",
        },
        {
          name: "Apellido",
          value: "lastName",
          orderable: true,
          type: "string",
        },
        {
          name: "Cedula",
          value: "document",
          orderable: true,
          CouldBeEdited: true,
          type: "number",
        },
        {
          name: "Genero",
          value: "gender",
          orderableByEvent: true,
          type: "string",
          custmonMinWitdh: "6.5rem",
        },
        {
          name: "Color por Genero",
          value: "customcolor",
        },
      ],
      data: [],
    };
  },
  created() {
    this.mockApi();
  },
  methods: {
    deleteMaleRule(){
      this.deleteRow(
        (row)=>{
          return row.gender === 'Masculino'
        }
      )
    },
    setNewRegistersQuantity(quantity) {
      this.page = 1;
      this.quantityPerPage = quantity;
      this.mockApi();
    },
    elementClicked(e){
      console.log(e)
    },
    manageOrderPicked(column) {
      if (column.columnValue === "gender") {
        this.manageCustomGenderFilter(column.desc);
      } else {
        this.customFilterGender = null;
      }
    },
    async ajaxModeForTable(tableInfo,callBack){
        const response=await api(null,tableInfo.page,tableInfo.quantityPerPage)
        callBack({ data:response.data, page:tableInfo.page, totalPages:response.totalPages, quantityPerPage:tableInfo.quantityPerPage })
    },
    async mockApi() {
      this.tableLoader = true;
      const data = await api(
        this.customFilterGender,
        this.page,
        100
      );
      this.tableLoader = false;
      this.totalPages = data.totalPages;
      this.data = data.data;
    },
    manageCustomGenderFilter(desc) {
      if (desc) {
        this.customFilterGender = "Femenino";
      } else {
        this.customFilterGender = "Masculino";
      }
    },
  },
  watch: {
    customFilterGender(value) {
      this.tableLoader = true;
      this.page = 1;
      this.mockApi({ gender: value });
    },
  },
};
</script>
<style lang="scss" scoped>
.tableCostumClass {
  max-width: 50%;
  margin: auto;
  max-height: 600px;
}
.ColorForPerson {
  width: 20px;
  height: 20px;
  display: block;
  margin: auto;
  border-radius: 2px;
}
.containImage {
  display: flex;
  align-items: center;
  justify-content: center;
  img {
    max-height: 30px;
    max-width: 40px;
    mix-blend-mode: multiply;
  }
}
.fitImgSex {
  max-height: 70%;
  margin: auto;
}
</style>